-- ============================================================
-- Block 3: Cumulative ARPU vs CAC Dynamics by Campaign
-- Description: cumulative ARPU per user to compare against CAC.
-- ============================================================

WITH users_campaigns AS (
...
10105, 10122, 10134, 10135])::int AS user_id, 'Campaign № 1' AS ads_campaign
    UNION ALL
...
10059, 10067, 10069, 10073, 10075, 10078, 10079, 10081, 10092, 10106, 10110, 10113, 10131])::int AS user_id, 'Campaign № 2' AS ads_campaign
), 

users as (select DISTINCT ads_campaign, users_campaigns.user_id, min(time)::date as start_date
        
from users_campaigns left join user_actions on users_campaigns.user_id=user_actions.user_id 
group by 1,2),
        
    start_cohorts as (SELECT 
    ads_campaign,
    ROUND(250000.0 / COUNT(DISTINCT user_actions.user_id), 2) AS cac,COUNT(DISTINCT user_actions.user_id) as us
    FROM users
...
day_rev as (select ads_campaign,  
date-start_date as day, sum(price) as rev
from 
users inner join user_actions using(user_id)  left join 
...
round(sum(rev) over (partition by ads_campaign order by  day)/us::numeric,2) as cumulative_arppu,cac
from  
day_rev inner join 
start_cohorts using(ads_campaign)
where ads_campaign='Campaign № 1'
order by 1,2
